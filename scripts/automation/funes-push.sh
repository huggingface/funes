...
#
# The memory to publish to is this script's first argument — `funes add <agent> <memory>` bakes it
# into the hook command (`funes-push.sh <org/repo>`). With no memory (a local-only install), the
# push hook isn't registered at all, so this script always runs with a memory.
#
...
# The Stop hook (funes-index.sh) keeps the LOCAL memory fresh per turn; this is the only
...
# Worker mode (re-exec): do the real work, already detached from the session. The memory rides
# through as $2.
if [ "${1:-}" = "--worker" ]; then
    worker "${2:-}"
    exit 0
...
# Foreground: the memory is our first argument. Drain the hook payload on stdin, hand off to a
# detached worker (carrying the memory), and return.
REMOTE="${1:-}"
cat >/dev/null
