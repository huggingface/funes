//! `funes ask codex` grounding: a real temp store's passages land in the prompt handed to the
...
#[tokio::test]
async fn grounding_embeds_store_passages_in_the_prompt() {
    ...
    let prompt = funes::ask::codex_grounding(funes::hub::Store::local(), "why reciprocal rank fusion", &|_| ())
        .await
        .unwrap();
    ...
    assert!(
        ...
        "the store's passage is embedded: {prompt}"
    );
