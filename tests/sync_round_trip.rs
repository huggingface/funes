...
//! (append → shallow-stage delta), recall both markers back from the remote, then delete the
...
async fn sync_round_trip_create_append_recall() {
    ...
    // create (first publish) → grow → append (shallow-stage delta) → recall both, all captured.
