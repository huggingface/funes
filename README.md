...
| `funes recall "…"` / `funes get …` / `funes sessions` / `funes scan …` | [docs/recall.md](docs/recall.md) — recall passages and drill into them; enumerate a memory's sessions; find a literal in every block |
...
- [docs/recall.md](docs/recall.md) — retrieval, drill-down with `get`, enumeration with `sessions`,
  exhaustive literal search with `scan`, and reading a shared memory.
