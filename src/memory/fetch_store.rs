...
impl FetchStore {
    ...
    /// Serve `location` from a copy obtained after discarding `stale`, at most one discard per read.
    async fn repair(&self, stale: &Path, location: &OPath, options: GetOptions) -> OSResult<GetResult> {
        if self.fetcher.discard(stale).await.is_ok() {
...
impl OSObjectStore for FetchStore {
    async fn get_opts(&self, location: &OPath, options: GetOptions) -> OSResult<GetResult> {
        ...
        match serve_from_file(&path, location, &options) {
            Ok(result) => Ok(result),
            Err(Unservable::Stale) => self.repair(&path, location, options).await,
...
fn serve_from_file(path: &Path, location: &OPath, options: &GetOptions) -> Result<GetResult, Unservable> {
    ...
    let range = match &options.range {
        ...
        Some(requested) => requested.as_range(total).map_err(|_| Unservable::Stale)?,
    };
...
mod tests {
    ...
    /// A stale copy no refetch repairs leaves the inner store to answer, after one discard.
    #[tokio::test]
