...
why we ruled out an approach, what the error actually was, the decision we finally
settled on — disappears when the session ends. Each new agent, on each new host,
starts from zero.
...
That diagnosis is right. But traces are only potential memory. A directory of session
logs — local or in a bucket — is still an archive. You cannot `grep` your way to
*“why did we move off the streaming parser?”* across ten thousand turns. For an agent
to use those traces while it works, they need indexing, retrieval, ranking, and exact
provenance.

That is what [`funes`](https://github.com/huggingface/funes) provides: durable memory
for Claude Code, Codex, pi, and Hermes, built from the sessions already on your
machine. It works locally, becomes part of your agent's normal workflow with one
command, and can be published as a Hugging Face dataset when you want it to travel.
...
turn. Indexing is incremental: new runs add new turns rather than embedding the whole
history again, while older and deeper content can backfill in bounded steps. With pi,
which has no hook system, `add` installs the read-side tools and `funes index` refreshes
...
a local Lance dataset. A query combines vector and BM25 search, fuses their rankings,
reranks the candidates with a cross-encoder, reweights them by recency, and attaches
neighboring chunks.
...
- **One memory across agents.** Claude Code, Codex, pi, and Hermes all write to the
  same shape. Recall spans their histories, and every hit says which agent produced it.
...
The stranger problem is already solved on one machine. But memory gets more useful
when the next agent is running somewhere else.
...
The agent now recalls from that memory. For agents with hooks, funes continues to
index locally each turn and publishes at session boundaries. Run the same command on
...
Face dataset repo you own. Publishing is also available directly:
...
already hosts open weights and open datasets. funes adds open working memory: the
decisions, failed approaches, and rationale behind a project, queryable by another
