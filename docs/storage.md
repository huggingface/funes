...
How big does a funes store get, and how does it grow? This document builds a
per-chunk cost model from three real stores and projects it forward.

All figures were measured with funes 0.8.0 against bge-small-en-v1.5 stores
(`DIM = 384`). Coefficients shift a little with chunk length and vocabulary, but
the shape holds.

## Reference stores

Two local stores on the same author, plus the published aggregate:
...
Both stores hold chunks of the **same size** — ~2.1 KB each. The main store is
larger only because it has more of them, not heavier ones; per-chunk cost is
essentially content-independent.

## What's on disk

A store is a single Lance dataset (`chunks.lance`) with these parts:
...
regardless of session type. The two stores agree within ~4%.
...
Both index types get *cheaper per chunk* as the store grows:
...
**→ budget ~0.18 KB/chunk of index at scale** (the 0.38 KB from the small store
is small-N overhead, not the trend).

### Total

A store costs roughly **~2.3 KB/chunk** at scale — ≈92% data, ≈8% index, and the
...
chunks/active-day, measured), a store accretes on the order of **~6 MB/day**,
...
A published store is an HF Hub dataset repo, so it counts against the account's
