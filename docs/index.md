...
`funes index` builds or updates your local store from session transcripts. [`funes add`](add.md)
runs it for you on every turn; run it by hand to seed a store, to fold in a new source, or to keep
recall fresh for an agent with no hooks (pi).

```bash
funes index      # a fast, text-first pass over every known harness dir, into one local store
```
...
one store, then offers to finish any deeper work left. Scope it to a single agent with `--harness`:
...
same chunks** no matter when it's indexed — and re-running embeds nothing already stored. That is
...
The embedding model is **pinned and stamped into the store**; querying with a different one is
refused. To change it, rebuild from the transcripts — the store is a disposable derived artifact, and
...
downstream is source-agnostic. A store runs ~2.3 KB/chunk and grows ~6 MB on a heavy day — see
[storage.md](storage.md).

## See also

- [recall.md](recall.md) — querying the store you just built.
- [automation.md](automation.md) — the per-turn indexing the hooks run.
- [storage.md](storage.md) — how a store grows on disk.
