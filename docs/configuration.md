...
| `FUNES_INTEGRATIONS` | Directory `funes add` installs an agent integration from, instead of the published one. funes can't vouch for those files, so it asks you to confirm before running them. A funes built from a checkout already uses that checkout's `integrations/`, with no prompt. |
